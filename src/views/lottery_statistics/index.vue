    ...
    <el-card class="card" shadow="hover">
      <el-form :model="params" ref="formRef" label-width="70px" :class="$store.state.isMobile ? 'el-form-m' : 'el-form-p'" :label-position="$store.state.isMobile ? 'top' : 'left'">
        ...
        <el-form-item label="公司" v-if="params.type == 'store'">
          <el-select v-model="params.store_id" placeholder="选择或搜索公司" clearable multiple collapse-tags :max-collapse-tags="3" collapse-tags-tooltip filterable @change="storeChange">
      ...
      <div class="tableData" style="padding-bottom: 15px" v-loading="loading" element-loading-text="数据加载中......">
        ...
        >
          ...
          <el-table-column prop="store_name" label="公司" min-width="100" />
          <el-table-column prop="award_1" label="特等奖" sortable min-width="100" />
          <el-table-column prop="award_2" label="一等奖" sortable min-width="100" />
...
const selectAllStore = (e) => {
  ...
  if (params.store_id.includes(0) && params.store_id.length - 1 < storeList.value.length) {
    params.store_id = companys
  } else {
    params.store_id = ''
  }
}
