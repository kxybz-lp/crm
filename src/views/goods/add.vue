          ...
          <el-tab-pane label="基础信息" name="basic">
            <el-form-item label="商品类型" prop="goods_type">
              <el-radio-group v-model="form.goods_type">
                ...
                <el-radio :label="20" size="large" border>虚拟商品/无需物流</el-radio>
              </el-radio-group>
            </el-form-item>
          ...
          <el-tab-pane label="规格库存" name="sku">
            ...
            <template v-if="form.spec_type === 10">
              ...
              <el-form-item label="商品价格">
                <el-input type="number" v-model="form.sku[0].goods_price"><template
                    #append>元</template></el-input>
              </el-form-item>
              <el-form-item label="划线价格">
                <el-input type="number" v-model="form.sku[0].line_price"><template
                    #append>元</template>
                </el-input>
              </el-form-item>
              <el-form-item label="商品库存">
                <el-input type="number" v-model="form.sku[0].stock_num"><template
                    #append>{{form.goods_unit}}</template></el-input>
              </el-form-item>
              <el-form-item label="商品重量">
                <el-input type="number" v-model="form.sku[0].goods_weight"><template
                    #append>KG</template>
                </el-input>
              </el-form-item>
            </template>
...
import { toast, array_column, removeDuplicatedItem } from '@/utils/utils'
import { specList, skuList } from '@/hooks/useSku.js'
import ChooseImage from '@/components/ChooseImage.vue'
import SkuCard from './components/SkuCard.vue'
import SkuTable from './components/SkuTable.vue'
import Editor from '@/components/Editor.vue'
import { initSpecList, initSkuList } from '@/hooks/useSku.js'
...
const submit = () => {
  formRef.value.validate((valid) => {
    ...
    if (form.spec_type == 10) {
      ...
      // specListArr.forEach((item) => {
      //   let spec_value_arr = removeDuplicatedItem(array_column('spec_value', item.valueList))
      //   if (spec_value_arr.length != item.valueList.length) {
      //     toast('不能存在重复的规格值 请检查您的输入', 'error')
      //     activeTab.value = 'sku'
      //     return
      //   }
      // })
    ...
    goods
      ...
      .then((res) => {
        if (res.code > 0) {
          ...
          initSkuList([])
          // 跳转到订单列表页，并重新获取数据
          let index = tabList.findIndex((item) => item.path === '/goods/add')
          router.replace({ path: '/goods/index', query: { reload: true } }).catch((err) => {})
