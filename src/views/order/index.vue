<template>
  <div class="app-container">
        ...
        <el-row :gutter="20">
          <el-col :md="6" :offset="0">
            <el-form-item label="关键词">
              <el-input v-model="params.keywords" style="max-width: 600px" placeholder="请输入关键词" class="input-with-select">
                <template #prepend>
                  <el-select v-model="params.keywords_type" placeholder="Select" style="width: 115px">
          ...
          <el-col :md="6" :offset="0">
            <el-form-item label="订单状态">
              <el-select v-model="params.order_type" placeholder="选择订单状态" clearable @clear="getData(1)">
          ...
          <el-col :md="6" :offset="0">
            <el-form-item label="收货公司">
              <el-select v-model="params.store_id" placeholder="选择收货公司" filterable clearable @clear="getData(1)">
      ...
      <ListHeader ref="headerRef" :rule="{ export: 194 }" @add="handleAdd" @export="exportExcel">
        <el-form class="search-form" :model="params" ref="searchRef" label-width="0px" size="default">
          <el-form-item label="" v-show="!showSearch && !$store.state.isMobile">
            <el-input v-model="params.order_no" placeholder="输入订单号" clearable @clear="getData(1)"> </el-input>
          </el-form-item>
          <el-form-item>
      ...
      >
        ...
        <el-table-column label="商品信息" width="360">
          <template #default="scope">
            <el-row v-for="item in scope.row.goods" :key="item.order_goods_id" align="middle" :gutter="30">
              <el-col :span="5">
                <el-image style="width: 60px; height: 60px" :src="item.goods_thumb" />
              </el-col>
              <el-col :span="19">
                <el-row style="overflow: hidden"
                  ><span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap">{{ item.goods_name }}</span></el-row
                >
                <el-row>
                  <span style="color: #999" v-for="itm in item.goods_props" :key="itm.id">{{ itm.value.name }}</span>
                </el-row>
                <el-row>
        ...
        <el-table-column prop="user_mobile" show-overflow-tooltip label="用户信息">
          <template #default="scope">
            ...
            >
            <el-row
              ><span>{{ scope.row.user_mobile }}</span></el-row
            >
          </template>
        </el-table-column>
        ...
        <el-table-column sortable label="交易状态">
          <template #default="scope">
            ...
            <el-row style="margin-bottom: 10px">
              <el-tag size="small" type="success" v-if="scope.row.delivery_status == 20">已发货</el-tag>
              <el-tag size="small" type="info" v-if="scope.row.delivery_status == 10">待发货</el-tag>
              <el-tag size="small" type="warning" v-if="scope.row.delivery_status == 30">部分发货</el-tag>
            </el-row>
            <el-row>
        ...
        <el-table-column label="操作" fixed="right" width="240">
          <template #default="scope">
            ...
            <el-button
              ...
              size="small"
              type="danger"
              @click="handleDeliver2(scope.row)"
            >
              发货
            </el-button>
            <el-button
            ...
            <el-dropdown v-permission="173">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item v-if="scope.row.pay_status == 10 && scope.row.order_status == 10" @click="handleUpdate(scope.row)">订单改价</el-dropdown-item>
                  <el-dropdown-item @click="handleCancel(scope.row)" v-if="scope.row.pay_status == 20 && scope.row.delivery_status == 10 && scope.row.order_status == 10">立即退款</el-dropdown-item>
                  <el-dropdown-item
                    @click="handleInvoice(scope.row)"
                    v-if="scope.row.pay_status == 20 && scope.row.delivery_status == 20 && (scope.row.order_status == 10 || scope.row.order_status == 30)"
                    >上传发票</el-dropdown-item
                  >
                  <el-dropdown-item @click="handleDelete(scope.row.order_id)">删除订单</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </template>
    ...
    <FormDialog destroyOnClose title="订单发货" ref="formDialogRef" @dialogClosed="dialogClosed" @submit="handleDeliverySubmit">
      <el-form :model="deliveryForm" ref="deliveryFormRef" :rules="deliverRules" label-width="140px" :label-position="$store.state.isMobile ? 'top' : 'right'">
        ...
        <el-form-item label="快递公司" prop="express_id" v-if="deliveryForm.delivery_method == 10">
          <el-select v-model="deliveryForm.express_id" placeholder="选择快递公司">
            <el-option :value="item.express_id" :label="item.express_name" v-for="item in expressList" :key="item.express_id"></el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog v-model="deliveryDialogVisible" title="订单发货" :width="$store.state.isMobile ? '90%' : '50%'" :destroy-on-close="true" @closed="deliveryDialogClosed" :close-on-click-modal="false">
      <el-scrollbar>
        <el-tabs v-model="activeName">
          <el-tab-pane label="未发货商品" name="notDelivery">
            <el-table
              :data="deliveryGoodsNotDelivery"
              ref="deliveryMultipleTableRef"
              style="width: 100%"
              :header-cell-style="{ color: '#2c3e50', backgroundColor: '#f2f2f2' }"
              :row-key="getRowKeys"
              @select="handleDeliverySelectionChange"
              @select-all="handleDeliverySelectionAll"
            >
              <el-table-column type="selection" prop="order_goods_id" :reserve-selection="true" width="55" />
                  ...
                  <div class="goods_info" style="display: flex; align-items: center">
                    ...
                    <div class="goods_info_text">
                      ...
                      <div class="goods_info_spec">
                        <span style="color: #999" v-for="itm in scope.row.goods_props" :key="itm.id">{{ itm.value.name }}</span>
                      </div>
                    </div>
                  </div>
                </template>
              ...
              <el-table-column label="发货数量" width="180">
                <template #default="scope">
                  <el-input-number v-model="scope.row.deliveryNum" :min="1" :max="scope.row.allowDeliveryNum" />
                </template>
              </el-table-column>
            </el-table>
            <div class="dialog-form" style="width: 80%; margin: 30px auto 0">
              <el-form :model="deliveryForm2" ref="deliveryFormRef2" :rules="deliverRules" label-width="140px" :label-position="$store.state.isMobile ? 'top' : 'right'">
                <!-- <el-form-item label="订单ID" prop="order_id">
                ...
                <el-form-item label="快递公司" prop="express_id" v-if="deliveryForm2.delivery_method == 10">
                  <el-select v-model="deliveryForm2.express_id" placeholder="选择快递公司">
                    <el-option :value="item.express_id" :label="item.express_name" v-for="item in expressList" :key="item.express_id"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item label="快递单号" prop="express_no" v-if="deliveryForm2.delivery_method == 10">
          ...
          <el-tab-pane label="已发货商品" name="isDelivery">
            <el-table :data="deliveryGoodsIsDelivery" style="width: 100%" :header-cell-style="{ color: '#2c3e50', backgroundColor: '#f2f2f2' }">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="deliverySubmit"> 确认 </el-button>
        </div>
      </template>
    </el-dialog>
    <!-- 审核取消订单 -->
    <FormDialog destroyOnClose title="审核取消订单" ref="cancelFormDialogRef" @submit="handleCancelSubmit">
      <el-form :model="cancelForm" ref="cancelFormRef" :rules="cancelRules" label-width="140px" :label-position="$store.state.isMobile ? 'top' : 'right'">
        <el-form-item label="订单ID" prop="order_id">
        ...
        <el-form-item label=""> 同意后将退回付款金额并关闭订单 </el-form-item>
      </el-form>
    </FormDialog>
    <!-- 订单改价 -->
    <FormDialog destroyOnClose title="订单改价" ref="updateFormDialogRef" @submit="handleUpdateSubmit">
      <el-form :model="updateForm" ref="updateFormRef" :rules="updateRules" label-width="140px" :label-position="$store.state.isMobile ? 'top' : 'right'">
        <el-form-item label="订单ID" prop="order_id">
    ...
    <!-- 上传发票 -->
    <FormDialog destroyOnClose title="上传发票" ref="invoiceFormDialogRef" @submit="handleInvoiceSubmit">
      <el-form :model="invoiceForm" ref="invoiceFormRef" :rules="invoiceRules" label-width="140px" :label-position="$store.state.isMobile ? 'top' : 'right'">
        <el-form-item label="订单ID" prop="order_id">
          <el-input show-word-limit v-model="invoiceForm.order_id" disabled></el-input>
        </el-form-item>
        <el-form-item label="订单发票" prop="invoice">
          <ChooseImage v-model="invoiceForm.invoice" />
        </el-form-item>
      </el-form>
    </FormDialog>
...
const handleInvoice = (order) => {
  invoiceForm.order_id = order.order_id
  invoiceForm.invoice = order.invoice
  invoiceFormDialogRef.value.openFormDialog()
}
const handleInvoiceSubmit = () => {
  invoiceFormRef.value.validate((valid) => {
    if (!valid) return
    order
      .update_invoice(invoiceForm.order_id, invoiceForm)
      .then((res) => {
        if (res.code > 0) {
          toast('发票上传成功')
          getData()
          invoiceFormDialogRef.value.closeFormDialog()
        } else {
          toast(res.message || 'error', 'error')
          return false
        }
